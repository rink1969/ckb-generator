...
processBinaryVote (ResolvedTransaction tx deps inputs LockScriptBinaryVote) = ResolvedTransaction new_tx [] [] LockScriptMultiSig where
  ...
  output_cap = foldl (+) 0 (map ((read :: String -> Integer) . _output_capacity. cell_with_status_cell_output . cell_with_status_cell) inputs)
  output_script = fetchOldOutputScript tx
  new_output = Output (printf "0x%x" (output_cap - minFee)) output_script Nothing
